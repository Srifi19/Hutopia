...
class AuthModel {
    ...
    async registerMainInfos(email, password) {
        try {
            ...
            await db.query('Call db_User_Register_Enterprise_mainInfos(?,?,?,?,?,?)', [email, password, UserId, UserNo, EnterpriseId, EnterpriseNo]);
    ...
    async registerContactPerson(UserId, firstName, lastName, type) {
        try {
            await db.query('Call db_User_Register_Enterprise_ContactPerson(?,?,?,?)', [UserId, firstName, lastName, type]);
        } catch (error) {
            throw error;
        }
    }
    ...
    async registerAddress(UserId, Country, City, postalCode, StreetAddress) {
        ...
        const LocationId = uuidv4();
           await db.query('Call db_User_Register_Enterprise_Address(?,?,?,?,?,?)', [LocationId , UserId, Country, City, postalCode, StreetAddress]);
        } catch (error) {
            console.log(error);
            throw error;
        }
