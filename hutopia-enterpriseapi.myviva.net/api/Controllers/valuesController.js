...
exports.getCertificatesByChars = async (req, res) => {
  ...
  await valuesModel
    .getCertificatesByChars(string, PageNumber, PageSize)
    .then((certificates) => {
      res.status(200).json({
        success: true,
...
exports.getAllPerks = async (req, res) => {
  await valuesModel
    ...
    .then((perks) => {
      ...
      res.status(200).json({
        success: true,
        message: "Perks successfully fetched",
        data: perks,
      });
    })
    .catch((err) => {
      ...
      res.status(500).json({
        success: false,
        message: "An error occurred while fetching perks",
      });
    });
};
...
exports.getAllSupplementalPay = async (req, res) => {
  await valuesModel
    ...
    .then((supplementalPay) => {
      res.status(200).json({
        success: true,
        message: "Supplemental pay information successfully fetched",
        data: supplementalPay,
      });
    })
    .catch((err) => {
      logger.error("Error: ", err);
      res.status(500).json({
        success: false,
        message:
          "An error occurred while fetching supplemental pay information",
      });
    });
};
