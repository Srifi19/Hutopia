    ...
    const Infos = req.body;
        await jobModel.CreateJob(userId, Infos)
            ...
            .catch((error) => {
                ...
                res.status(400).json({success: false, message: 'Something went wrong while creating the job.'});
            })
...
exports.GetAllJobsForEnterprise = async (req, res) => {
    ...
    try {
        ...
        logger.error('Error: ', error);
        res.status(400).json({success: false, message: 'Something went wrong while fetching jobs.'});
    }
};
...
exports.GetJobInformations = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({success: true, message: 'Job information successfully fetched.', content: result});
    } catch (error) {
        logger.error('Error: ', error);
        res.status(400).json({success: false, message: 'Something went wrong while fetching job information.'});
    }
};
...
exports.GetJobDetailedInformations = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true, message: 'Detailed job information successfully fetched.', content: result
        });
    } catch (error) {
        logger.error('Error: ', error);
        res.status(400).json({
            success: false, message: 'Something went wrong while fetching detailed job information.'
        });
    }
};
...
exports.CreateJobSchedule = async (req, res) => {
    ...
    await jobModel.CreateJobSchedule(userId, jobId, jobSchedule)
        .then(() => {
            res.status(200).json({success: true, message: 'Job schedule created successfully.'});
        })
        .catch((error) => {
            logger.error('Error: ', error);
            res.status(400).json({success: false, message: 'Something went wrong while creating job schedule.'});
        })
